...
export class AuthService {
  ...
  login(user:User):Observable<User>{
    ...
    return this.http.post<User>(`${environment.fbApiUrl}:signInWithPassword?key=${environment.apiKey}`,body)
      .pipe(
  ...
  signUp(user:User):Observable<User>{
    ...
    return this.http.post<User>(`${environment.fbApiUrl}:signUp?key=${environment.apiKey}`,body)
      .pipe(
        tap(this.setToken||console.error),
        catchError(this.handleError.bind(this))
      )
  ...
  getAuthInfo(){
    return this.http.post<FbUserResponse>(`${environment.fbApiUrl}:lookup?key=${environment.apiKey}`,{idToken:this.token})
      .pipe(
        map((val)=>{
